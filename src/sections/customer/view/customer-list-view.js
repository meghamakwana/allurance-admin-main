...
import { CUSTOMER_ENDPOINT } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../customer-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(CUSTOMER_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${CUSTOMER_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${CUSTOMER_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleViewRow = useCallback((id) => {
    ...
    // router.push(paths.dashboard.customer.account);
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 50,
      hideable: false,
      renderCell: (params) => <RenderCellDOB params={params} />,
    },
    {
      field: 'total_orders',
      ...
      minWidth: 80,
      hideable: false,
      renderCell: (params) => <RenderCellOrders params={params} />,
    },
    {
      field: 'status',
      headerName: 'Status',
